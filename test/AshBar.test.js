...
const CMBSTToken = artifacts.require('CMBSTToken');
const AshBar = artifacts.require('AshBar');

contract('AshBar', ([alice, bob, carol, dev, minter]) => {
  beforeEach(async () => {
    this.cake = await CMBSTToken.new({ from: minter });
    this.syrup = await AshBar.new(this.cake.address, { from: minter });
  });
